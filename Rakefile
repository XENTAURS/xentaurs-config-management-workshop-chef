...
namespace :ec2 do
  ...
  desc 'List available EC2 kitchen instances'
  task :list, :profile  do |_, args|
    args.with_defaults(profile: '')
    current_local = ENV['KITCHEN_LOCAL_YAML']
    ENV['KITCHEN_LOCAL_YAML'] = '.kitchen.ec2.yml'
    results = `kitchen list #{args.profile}`
    puts results
    ENV['KITCHEN_LOCAL_YAML'] = current_local
  end
  ...
  desc 'Login to Existing EC2 instance'
  task :login, :profile  do |_, args|
    args.with_defaults(profile: 'default')
    Kitchen.logger = Kitchen.default_file_logger
    @loader = Kitchen::Loader::YAML.new(local_config: './.kitchen.ec2.yml')
  ...
  desc 'Test EC2 instance end to end'
  task :test, :profile  do |_, args|
    args.with_defaults(profile: 'default')
    Kitchen.logger = Kitchen.default_file_logger
    @loader = Kitchen::Loader::YAML.new(local_config: './.kitchen.ec2.yml')
    config = Kitchen::Config.new(loader: @loader)
  ...
  desc 'Diagnose Kitchen-EC2 configs'
  task :diagnose, :profile  do |_, args|
    args.with_defaults(profile: 'default')
    Kitchen.logger = Kitchen.default_file_logger
    @loader = Kitchen::Loader::YAML.new(local_config: './.kitchen.ec2.yml')
    config = Kitchen::Config.new(loader: @loader)
    instances = config.instances.get_all(/#{args.profile}/)
    puts Kitchen::Diagnostic.new(instances: instances).read.to_yaml
  end
end
